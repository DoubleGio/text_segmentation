...
class TS2_Model(nn.Module):
    ...
    def __init__(self, sentence_encoder: SentenceEncodingRNN2, hidden=128, num_layers=2):
        ...
        self.sentence_lstm = nn.LSTM(
            input_size=sentence_encoder.hidden * 2 + 768, # Output of BiLSTM hidden state + the dimension of BERT embedding.
            hidden_size=hidden,
        ...
        self.sentence_lstm_2 = nn.LSTM(
            input_size=sentence_encoder.hidden * 4,
            hidden_size=hidden,
            num_layers=num_layers,
            batch_first=True,
            dropout=0,
    ...
    def compute_sentence_pair_coherence(self, doc_out: torch.TensorType, pair_size=1):
        """
        Does something...
        TODO: This isnt the sigma(cos(e1, e2)) as described in the paper (in the final step)? What do we even do here?
        """
    ...
    def compute_sentences_similarity(self, X, sent_i, win_size):
        """
        TODO: Clear up the calculations and variable names.
        """
        X1 = X.unsqueeze(0)
        Y1 = X.unsqueeze(1)
    ...
    def forward(
        ...
        # TODO: Implement a batched/optimized version of this.
        win_size = 3
        doc_outputs = []
        sim_scores = []
        doc_start_index = 0
...
def supervised_cross_entropy(
    ...
    """
    Computes the supervised cross entropy loss together with the loss from the auxiliary task.
    """
